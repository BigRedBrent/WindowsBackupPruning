...
If $CmdLine[0] And $CmdLine[1] And Number($CmdLine[1]) >= 2 Then $BackupsToKeep = Number($CmdLine[1])
...
If @OSArch <> "X86" Then $Key = StringReplace($Key, "HKEY_LOCAL_MACHINE", "HKEY_LOCAL_MACHINE64")

Local $BackupDir = RegRead($Key, "PresentableName")
If @error <> 0 Then Exit MsgBox($MB_ICONWARNING, $Title, "Backup drive was not found!")

$BackupDir &= @ComputerName
If Not FileExists($BackupDir) Then Exit MsgBox($MB_ICONWARNING, $Title, "Backup folder was not found!")
